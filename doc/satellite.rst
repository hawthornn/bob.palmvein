...
The simplest and most clean way to test your own code in the PalmveinRecLib is to add it in a satellite package.
In principle, you can choose a name that fits you, but it is preferable to have a package name that starts with ``xPalmveinRecLib.`` to show that it is a satellite package to the PalmveinRecLib.
...
In case, your contribution is a palmvein recognition algorithm, you might want to :ref:`register it <register-resources>`.
After doing that, you can simply use the ``bin/palmveinverify.py`` (or any other script of the PalmveinRecLib) with your registered tool, as if it would be part of the PalmveinRecLib.
